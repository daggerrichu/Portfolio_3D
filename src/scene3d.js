...
export class PortfolioScene3D {
  ...
  /**
   * Sovereign Flight Path Scroll Keyframes for Winged Demon
   * Continuous Wing Flapping active at all times; mouse head follow removed.
   */
  getScrollKeyframeTransform() {
    const isMobile = window.innerWidth < 768;

    const keyframes = [
      // Hero (0.00): High Right Airborne Hover — Wings flapping, front view
      ...
      // Skills (0.80): Center Stage Spellcasting Hover — High elevated central flight
      { p: 0.80, posX: 0.0,                 posY: isMobile ? 0.3 : 0.4,  posZ: 0.4, rotX: 0.1,   rotY: Math.PI * 0.15, rotZ: 0.0,   scale: 1.1 },
      // Contact (1.00): Demon Sovereign Final Stance — Centered full wing-span front view
  ...
  animate() {
    ...
    if (activeGroup) {
      ...
      // Rotation Interpolation along Flight Path (Mouse follow removed)
